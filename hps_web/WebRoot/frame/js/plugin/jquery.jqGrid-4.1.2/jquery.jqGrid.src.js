...
/**
 * @license jqGrid 4.1.2 - jQuery Grid Copyright (c) 2008, Tony Tomov,
 *          tony@trirand.com Dual licensed under the MIT and GPL licenses
 *          http://www.opensource.org/licenses/mit-license.php
 *          http://www.gnu.org/licenses/gpl-2.0.html Date: 2011-07-20
 */
// jsHint options
...
(function($) {
		...
		from : function(source, initalQuery) {
			...
			var queryObject = function(d, q) {
				...
				this._compareValues = function(func, f, v, how, t) {
					...
					// var val=v===null?f:v,
					var val = v, swst = t.stype === undefined
							? "text"
							: t.stype;
					if (v !== null) {
						switch (swst) {
				...
				this.startsWith = function(f, v) {
					...
					if (_useProperties) {
						self._append(self
								._getStr('jQuery.jgrid.getAccessor(this,\'' + f
										+ '\')')
								+ '.substr(0,'
								+ length
								+ ') == '
		...
		return this.each(function() {
			...
			var grid = {
				...
				dragStart : function(i, x, y) {
					...
					this.curGbox = $("#rs_m" + $.jgrid.jqID(p.id), "#gbox_"
									+ $.jgrid.jqID(p.id));
					this.curGbox.css({
								display : "block",
								left : y[0],
					...
					if (table[0].rows.length) {
						try {
							...
							rh = rows ? $(rows).outerHeight()
									|| grid.prevRowHeight : grid.prevRowHeight;
						} catch (pv) {
			...
			var gv = $("<div class='ui-jqgrid-view'></div>"), ii, isMSIE = $.browser.msie
					? true
					: false, isSafari = $.browser.webkit || $.browser.safari
					? true
					: false;
			ts = this;
			...
			var sortkeys = ["shiftKey", "altKey", "ctrlKey"], intNum = function(
				...
				if (ts.p.deepempty) {
					$("#" + $.jgrid.jqID(ts.p.id) + " tbody:first tr:gt(0)")
							.remove();
				} else {
					var trf = $("#" + $.jgrid.jqID(ts.p.id)
							+ " tbody:first tr:first")[0];
					$("#" + $.jgrid.jqID(ts.p.id) + " tbody:first").empty()
							.append(trf);
				}
				...
				var startReq = new Date(), locdata = (ts.p.datatype != "local" && ts.p.loadonce)
						|| ts.p.datatype == "xmlstring", xmlid = "_id_", frd = ts.p.datatype == "local"
						? "local"
						: "xml";
				if (locdata) {
					ts.p.data = [];
					...
					while (j < gl) {
						...
						if (ts.p.xmlReader.repeatitems) {
							...
							$.each(F, function(k) {
										...
										rowData.push(addCell(rid, v, k + gi
														+ si + ni, j + rcnt,
												xmlr));
									});
						} else {
				...
				var ir = 0, v, i, j, f = [], F, cur, gi = 0, si = 0, ni = 0, len, drows, idn, rd = {}, fpos, idr, rowData = [], cn = (ts.p.altRows === true)
						? " " + ts.p.altclass
						: "", cn1, lp;
				...
				if (locdata) {
					while (ir < len && drows[ir]) {
						...
						if (idr === undefined) {
							...
							if (f.length === 0) {
								if (dReader.cell) {
									var ccur2 = $.jgrid.getAccessor(cur,
											dReader.cell);
									idr = ccur2[idn] || idr;
									ccur2 = null;
								}
				...
				var compareFnMap = {
					...
					'ni' : function(queryObj, op) {
						return op === "OR" ? queryObj.orNot().equals : queryObj
								.andNot().equals;
					},
					...
					'nn' : function(queryObj, op) {
						return op === "OR" ? queryObj.orNot().isNull : queryObj
								.andNot().isNull;
					}
				...
				var cp, last, base, from, to, tot, fmt, pgboxes = "", sppg, tspg = ts.p.pager
						? "_" + $.jgrid.jqID(ts.p.pager.substr(1))
				...
				if (pgboxes) {
					...
					if (ts.p.pginput === true) {
						...
						sppg = ts.p.toppager ? '#sp_1' + tspg + ",#sp_1"
								+ tspg_t : '#sp_1' + tspg;
						$(sppg).html($.fmatter ? $.fmatter.util.NumberFormat(
								ts.p.lastpage, fmt) : ts.p.lastpage);
							...
							$.ajax($.extend(
									{
										...
										data : $
												...
												? ts.p.serializeGridData.call(
														ts, ts.p.postData)
												: ts.p.postData,
										success : function(data, st) {
											if (dt === "xml") {
												addXmlData(data, ts.grid.bDiv,
														rcnt, npage > 1, adjust);
											} else {
												addJSONData(data, ts.grid.bDiv,
														rcnt, npage > 1, adjust);
											}
											if (lc) {
												lc.call(ts, data);
				...
				if (ts.p.pgbuttons === true) {
					...
					$("#first" + $.jgrid.jqID(tp) + ", #prev"
							+ $.jgrid.jqID(tp) + ", #next" + $.jgrid.jqID(tp)
							+ ", #last" + $.jgrid.jqID(tp)).click(function(e) {
				...
				$("tr th:eq(" + idxcol + ") span.ui-icon-" + ts.p.sortorder,
						thd).removeClass('ui-state-disabled');
				$("tr th:eq(" + idxcol + ")", thd)
				...
				if (ts.p.subGrid && ts.p.datatype == 'local') {
					$("td.sgexpanded", "#" + $.jgrid.jqID(ts.p.id)).each(
							function() {
								$(this).trigger("click");
							});
				}
				...
				var initwidth = 0, brd = isSafari ? 0 : ts.p.cellLayout, vc = 0, lvc, scw = ts.p.scrollOffset, cw, hs = false, aw, gw = 0, cl = 0, cr;
			...
			for (i = 0; i < ts.p.colModel.length; i++) {
				ts.p.colModel[i] = $.extend(true, {}, ts.p.cmTemplate,
						ts.p.colModel[i].template || {}, ts.p.colModel[i]);
			...
			var hTable = $("<table class='ui-jqgrid-htable' style='width:"
					+ ts.p.tblwidth + "px' role='grid' aria-labelledby='gbox_"
					+ this.id
			...
			$(ts).before(grid.hDiv).click(function(e) {
				...
				var scb = $(td).hasClass("cbox"), cSel = true;
				...
				if (cSel === true) {
					if (ts.p.cellEdit === true) {
						...
						if (ts.p.multiselect && ts.p.multiboxonly) {
							if (scb) {
								...
								$(ts.p.selarrrow).each(function(i, n) {
									...
									$("#jqg_" + $.jgrid.jqID(ts.p.id) + "_"
											+ $.jgrid.jqID(n)).attr("checked",
											false);
								});
								ts.p.selarrrow = [];
						...
						if (e[ts.p.multikey]) {
							...
							scb = $("#jqg_" + $.jgrid.jqID(ts.p.id) + "_"
									+ ptr[0].id).attr("checked");
							$("#jqg_" + $.jgrid.jqID(ts.p.id) + "_" + ptr[0].id)
									.attr("checked", !scb);
						}
					}
			...
			if ($.isFunction(this.p.ondblClickRow)) {
				$(this).dblclick(function(e) {
							...
							ri = ptr[0].rowIndex;
							ci = $.jgrid.getCellIndex(td);
							ts.p.ondblClickRow.call(ts, $(ptr).attr("id"), ri,
									ci, e);
							return false;
						});
			}
			...
			$(grid.cDiv)
					...
					.append("<span class='ui-jqgrid-title"
							+ (dir == "rtl" ? "-rtl" : "") + "'>"
							+ ts.p.caption + "</span>")
					.addClass("ui-jqgrid-titlebar ui-widget-header ui-helper-clearfix");
					...
					$(".ui-jqgrid-titlebar-close", grid.cDiv).click(
							function(e) {
								...
								if (ts.p.gridstate == 'visible') {
									$(elems, "#gbox_" + $.jgrid.jqID(ts.p.id))
	...
	$.jgrid.extend({
				...
				if (!$t.p.multiselect) {
					...
					if (ia === -1) {
						...
						$("#jqg_" + $.jgrid.jqID($t.p.id) + "_"
								+ $.jgrid.jqID($t.p.selrow)).attr("checked",
								stat);
		...
		resetSelection : function(rowid) {
			return this.each(function() {
				...
				if (typeof(rowid) !== "undefined") {
					...
					$("#" + $.jgrid.jqID(t.p.id) + " tbody:first tr#"
							+ $.jgrid.jqID(sr))
							.removeClass("ui-state-highlight").attr(
									"aria-selected", "false");
					if (t.p.multiselect) {
						$("#jqg_" + $.jgrid.jqID(t.p.id) + "_"
								+ $.jgrid.jqID(sr)).attr("checked", false);
					...
					$(t.p.selarrrow).each(function(i, n) {
						...
						$(ind).removeClass("ui-state-highlight").attr(
								"aria-selected", "false");
						$("#jqg_" + $.jgrid.jqID(t.p.id) + "_"
								+ $.jgrid.jqID(n)).attr("checked", false);
					});
					$("#cb_" + $.jgrid.jqID(t.p.id)).attr("checked", false);
					t.p.selarrrow = [];
				}
						...
						if (t.p.datatype == 'local') {
							...
							if (t.p.treeGrid) {
								for (var key in t.p.treeReader) {
									if (lcdata
											.hasOwnProperty(t.p.treeReader[key])) {
		...
		footerData : function(action, data, format) {
			...
			this.each(function() {
						...
						$(this.p.colModel).each(function(i) {
							...
							if (action == "set") {
								if (data[nm] !== undefined) {
									vl = format ? t.formatter("", data[nm], i,
											data, 'edit') : data[nm];
									title = this.title ? {
										"title" : $.jgrid.stripHtml(vl)
									} : {};
									$("tr.footrow td:eq(" + i + ")",
											t.grid.sDiv).html(vl).attr(title);
									success = true;
								}
							} else if (action == "get") {
								res[nm] = $("tr.footrow td:eq(" + i + ")",
										t.grid.sDiv).html();
							}
						});
					});
		...
		showHideCol : function(colname, show) {
			return this.each(function() {
						var $t = this, fndh = false, brd = $.browser.webkit
								|| $.browser.safari ? 0 : $t.p.cellLayout, cw;
						if (!$t.grid) {
							return;
						}
						...
						$(this.p.colModel).each(function(i) {
							if ($.inArray(this.name, colname) !== -1
								...
								$("tr", $t.grid.hDiv).each(function() {
									$(this).children("th:eq(" + i + ")").css(
											"display", show);
								});
								$($t.rows).each(function(j) {
									$(this).children("td:eq(" + i + ")").css(
											"display", show);
								});
								if ($t.p.footerrow) {
									$("tr.footrow td:eq(" + i + ")",
											$t.grid.sDiv).css("display", show);
								}
								cw = this.widthOrg ? this.widthOrg : parseInt(
										this.width, 10);
								if (show === "none") {
									$t.p.tblwidth -= cw + brd;
								} else {
		...
		remapColumns : function(permutation, updateCells, keepHeader) {
			...
			if (updateCells) {
				resortRows($("#" + $.jgrid.jqID(ts.p.id) + " tbody:first"),
		...
		setGridWidth : function(nwidth, shrink) {
			return this.each(function() {
				...
				var $t = this, cw, initwidth = 0, brd = $.browser.webkit
						|| $.browser.safari ? 0 : $t.p.cellLayout, lvc, vc = 0, hs = false, scw = $t.p.scrollOffset, aw, gw = 0, cl = 0, cr;
			...
			return this.each(function() {
				...
				var triggerToolbar = function() {
					...
					if (p.stringResult === true || $t.p.datatype == "local") {
						var ruleGroup = "{\"groupOp\":\"" + p.groupOp
								+ "\",\"rules\":[";
						var gi = 0;
				...
				var clearToolbar = function(trigger) {
					...
					$.each($t.p.colModel, function(i, n) {
						v = (this.searchoptions && this.searchoptions.defaultValue)
						...
						switch (this.stype) {
							case 'select' :
								var v1;
								$("#gs_" + $.jgrid.jqID(nm) + " option",
										$t.grid.hDiv).each(function(i) {
											if (i === 0) {
												this.selected = true;
											}
											if ($(this).text() == v) {
					...
					if (jElem[0]) {
						jQuery.each(events, function() {
									if (this.data !== undefined) {
										jElem.bind(this.type, this.data,
												this.fn);
									} else {
					...
					if (this.search) {
						switch (this.stype) {
							case "select" :
								...
								if (surl) {
									// data returned should have already
									// constructed html select
...
(function($) {
	...
	var s = 0, H = $.jqm.hash, A = [], ie6 = $.browser.msie
		...
		var i = $('<iframe src="javascript:false;document.write(\'\');" class="jqm"></iframe>')
				.css({
							opacity : 0
						});
		if (ie6)
...
(function($) {
	...
	var J = $.jqDnR, M = J.dnr, E = J.e, E1, i = function(e, h, k, aR) {
		return e.each(function() {
					...
					h.bind('mousedown', {
								...
								E = d.e;
								E1 = aR ? $(aR) : false;
								// attempt utilization of dimensions plugin to
								// fix IE issues
								...
								if (E1 && d.k != 'd') {
									M1 = {
										X : p.left || f1('left') || 0,
										Y : p.top || f1('top') || 0,
										W : E1[0].offsetWidth || f1('width')
												|| 0,
										H : E1[0].offsetHeight || f1('height')
												|| 0,
										pX : v.pageX,
										pY : v.pageY,
										k : d.k
...
(function($) {
	...
	$.fmatter.util = {
		...
		DateFormat : function(format, date, newformat, opts) {
			var token = /\\.|[dDjlNSwzWFmMntLoYyaABgGhHisueIOPTZcrU]/g, timezone = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g, timezoneClip = /[^-+\dA-Z]/g, msDateRegExp = new RegExp("^\/Date\\((([-+])?[0-9]+)(([-+])([0-9]{2})([0-9]{2}))?\\)\/$"), msMatch = ((typeof date === 'string')
					? date.match(msDateRegExp)
	...
	$.fn.fmatter.rowactions = function(rid, gid, act, pos) {
		...
		var saverow = function(rowid) {
			...
			$(
					"tr#" + rid + " div.ui-inline-edit, " + "tr#" + rid
							+ " div.ui-inline-del",
					"#" + gid + ".ui-jqgrid-btable:first").show();
			$(
					"tr#" + rid + " div.ui-inline-save, " + "tr#" + rid
							+ " div.ui-inline-cancel",
					"#" + gid + ".ui-jqgrid-btable:first").hide();
		...
		switch (act) {
			case 'edit' :
				...
				break;
			case 'save' :
				if ($('#' + gid).jqGrid('saveRow', rid, op.onSuccess, op.url,
	...
	$.fn.fmatter.actions = function(cellval, opts, rwd) {
		...
		if (op.editformbutton) {
			ocl = "onclick=$.fn.fmatter.rowactions('"
					+ rowid
					+ "','"
					+ opts.gid
					+ "','formedit',"
	...
	$.unformat = function(cellval, options, pos, cnt) {
		...
		if (typeof unformatFunc !== 'undefined' && $.isFunction(unformatFunc)) {
			...
			switch (formatType) {
				...
				case 'currency' :
					...
					stripTag = new RegExp(sep, "g");
					ret = $(cellval).text().replace(stripTag, '').replace(
							op.decimalSeparator, '.').replace(op.prefix, '')
							.replace(op.suffix, '');
					break;
				case 'checkbox' :
		...
		return ret !== undefined ? ret : cnt === true
				? $(cellval).text()
	...
	$.extend($.jgrid, {
		...
		createModal : function(aIDs, content, p, insertSelector, posSelector,
			...
			$("a.ui-jqdialog-titlebar-close", mh).click(function(e) {
						var oncm = $("#" + aIDs.themodal).data("onClose")
								|| p.onClose;
						var gboxclose = $("#" + aIDs.themodal).data("gbox")
								|| p.gbox;
		...
		createEl : function(eltype, options, vl, autowidth, ajaxso) {
			...
			function setAttributes(elm, atr, exl) {
				var exclude = ['dataInit', 'dataEvents', 'dataUrl',
						'buildSelect', 'sopt', 'searchhidden', 'defaultValue',
						'attr'];
						...
						$.ajax($.extend({
							...
							success : function(data, status) {
								var a, ovm = [], elem = this.elem, vl = this.vl, options = $
										.extend({}, this.options), msl = options.multiple === true;
		...
		checkValues : function(val, valref, g, customobject, nam) {
			 ...
			 * "field": "name", "op": "eq", "data": "England" }, { "field":
			 * "id", "op": "le", "data": "5"} ] } ], "rules": [ { "field":
			 * "name", "op": "eq", "data": "Romania" }, { "field": "id", "op":
			 * "le", "data": "1"} ] }
			 */
...
(function($) {
	...
	$.fn.jqFilter = function(arg) {
		...
		return this.each(function() {
			...
			if (this.p.showQuery) {
				$(this)
						.append("<table class='queryresult ui-widget ui-widget-content' style='display:block;max-width:440px;border:0px none;'><tbody><tr><td class='query'></td></tr></tbody></table>");
			}
			/*
			 * Perform checking.
			 * 
				...
				if ($.isFunction(colModelItem.searchrules)) {
					...
					try {
						ret = $.jgrid.checkValues(val, -1, null,
								colModelItem.searchrules, colModelItem.label);
					} catch (e) {
					}
				}
			...
			this.onchange = function() {
				...
				this.p.error = false;
				this.p.errmsg = "";
				return $.isFunction(this.p.onChange) ? this.p.onChange.call(
						this, this.p) : false;
			};
			/*
			...
			/*
			 * Creates a grouping data for the filter @param group - object
			...
			this.createTableForGroup = function(group, parentgroup) {
				...
				if (parentgroup === null) {
					$(table)
							.append("<tr class='error' style='display:none;'><th colspan='5' class='ui-state-error' align='left'></th></tr>");
				}

				var tr = $("<tr></tr>");
				$(table).append(tr);
				// this header will hold the group operator type and group
				// action buttons for
				// creating subgroup "+ {}", creating rule "+" or deleting the
				// group "-"
				...
				if (this.p.ruleButtons === true) {
					...
					// populate dropdown with all posible group operators: or,
					// and
					var str = "", selected;
					for (i = 0; i < p.groupOps.length; i++) {
						selected = group.groupOp === that.p.groupOps[i]
								? " selected='selected'"
								: "";
						str += "<option value='" + that.p.groupOps[i] + "'"
								+ selected + ">" + that.p.groupOps[i]
								+ "</option>";
					}
				...
				if (this.p.ruleButtons === true) {
					...
					inputAddRule.bind('click', function() {
						...
						for (i = 0; i < that.p.columns.length; i++) {
							...
							// fields
							var searchable = (typeof that.p.columns[i].search === 'undefined')
									? true
						...
						// for the moment no change have been made to the rule,
						// so
				...
				if (group.groups !== undefined) {
					for (i = 0; i < group.groups.length; i++) {
						...
						tdMainHolderForSubgroup.append(this
								.createTableForGroup(group.groups[i], group));
				...
				if (group.rules !== undefined) {
					for (i = 0; i < group.rules.length; i++) {
						table.append(this.createTableRowForRule(group.rules[i],
										group));
					}
				}
			...
			this.createTableRowForRule = function(rule, group) {
				...
				ruleFieldSelect.bind('change', function() {
							...
							var elm = $.jgrid.createEl(cm.inputtype,
				...
				for (i = 0; i < that.p.columns.length; i++) {
					...
					if ((ignoreHiding && searchable) || (searchable && !hidden)) {
						...
						str += "<option value='" + that.p.columns[i].name + "'"
								+ selected + ">" + that.p.columns[i].label
								+ "</option>";
					}
				}
				...
				cm = p.columns[j];
				// create it here so it can be referentiated in the onchange
				// event
				...
				for (i = 0; i < op.length; i++) {
					ina = $.inArray(op[i], aoprs);
					if (ina !== -1) {
						selected = rule.op === that.p.ops[ina].name
								? " selected='selected'"
								: "";
						str += "<option value='" + that.p.ops[ina].name + "'"
								+ selected + ">" + that.p.ops[ina].description
								+ "</option>";
					}
				}
				...
				$(ruleDataInput).addClass("input-elm").bind('change',
						function() {
							rule.data = $(this).val();
							that.onchange(); // signals that the filter has
												// changed
			...
			this.getStringForRule = function(rule) {
				var opUF = "", opC = "", i, cm, ret, val, numtypes = ['int',
						'integer', 'float', 'number', 'currency']; // jqGrid
				...
				if ($.inArray(cm.searchtype, numtypes) !== -1 || opC === 'nn'
						|| opC === 'nu') {
	...
	$.jgrid.extend({
		searchGrid : function(p) {
			...
			return this.each(function() {
				...
				if ($("#" + IDs.themodal).html() !== null) {
					...
					var fil = $("<span><div id='"
							+ fid
							+ "' class='searchFilter' style='overflow:auto'></div></span>")
							.insertBefore("#gview_" + $t.p.id);
					...
					if (!p.columns.length) {
						$.each(columns, function(i, n) {
							...
							// find first searchable column and set it if no
							// default filter
							if (!found) {
								var searchable = (typeof n.search === 'undefined')
										? true
					...
					if ((!defaultFilters && colnm)
						...
						var cmop = "eq";
						if (cmi >= 0 && columns[cmi].searchoptions
					...
					bt = "<table class='EditTable' style='border:0px none;margin-top:5px' id='"
							+ fid
							+ "_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content' style='margin:1px'/></td></tr><tr><td class='EditButton' style='text-align:left'>"
							+ bC
							+ tmpl
					...
					if (p.layer)
						$.jgrid.createModal(IDs, fil, p, "#gview_" + $t.p.id,
								$("#gbox_" + $t.p.id)[0], "#" + p.layer, {
									position : "relative"
								});
					else
						$.jgrid.createModal(IDs, fil, p, "#gview_" + $t.p.id,
					...
					if (p.stringResult === undefined) {
						// to provide backward compatibility, inferring
						// stringResult value from multipleSearch
					...
					$("#" + fid + "_search").bind('click', function() {
						...
						if (p.stringResult) {
							...
							if (typeof(res) === "string") {
								sdata[p.sFilter] = res;
								$.each([p.sField, p.sValue, p.sOper],
										function() {
											sdata[this] = "";
										});
							}
		...
		editGridRow : function(rowid, p) {
			...
			return this.each(function() {
				...
				var gID = $t.p.id, frmgr = "FrmGrid_" + gID, frmtb = "TblGrid_"
						+ gID, IDs = {
					themodal : 'editmod' + gID,
					modalhead : 'edithd' + gID,
					modalcontent : 'editcnt' + gID,
					scrollelm : frmgr
				}, onBeforeShow = $.isFunction(rp_ge[$t.p.id].beforeShowForm)
						? rp_ge[$t.p.id].beforeShowForm
						: false, onAfterShow = $
						.isFunction(rp_ge[$t.p.id].afterShowForm)
						? rp_ge[$t.p.id].afterShowForm
						: false, onBeforeInit = $
						.isFunction(rp_ge[$t.p.id].beforeInitData)
						? rp_ge[$t.p.id].beforeInitData
						: false, onInitializeForm = $
						.isFunction(rp_ge[$t.p.id].onInitializeForm)
						? rp_ge[$t.p.id].onInitializeForm
						: false, copydata = null, showFrm = true, maxCols = 1, maxRows = 0, postdata, extpost, newData, diff;
				if (rowid === "new") {
					rowid = "_empty";
				...
				function getFormData() {
					$("#" + frmtb + " > tbody > tr > td > .FormElement").each(
							function(i) {
								var celm = $(".customelement", this);
								if (celm.length) {
									var elem = celm[0], nm = $(elem)
											.attr('name');
									$.each($t.p.colModel, function(i, n) {
										if (this.name === nm
												&& this.editoptions
				...
				function createData(rowid, obj, tb, maxcols) {
					...
					$(obj.p.colModel).each(function(i) {
						...
						if (nm !== 'cb' && nm !== 'subgrid'
								&& this.editable === true && nm !== 'rn') {
							if (ind === false) {
								tmp = "";
							} else {
								if (nm == obj.p.ExpandColumn
										&& obj.p.treeGrid === true) {
									tmp = $("td:eq(" + i + ")", obj.rows[ind])
											.text();
								} else {
									try {
										tmp = $.unformat($("td:eq(" + i + ")",
														obj.rows[ind]), {
													rowId : rowid,
													colModel : this
												}, i);
							...
							var opt = $.extend({}, this.editoptions || {}, {
										...
										rowabove : false,
										rowcontent : ''
									}, this.formoptions || {}), rp = parseInt(
									frmopt.rowpos, 10)
									|| cnt + 1, cp = parseInt((parseInt(
											frmopt.colpos, 10) || 1)
											* 2, 10);
							if (rowid == "_empty" && opt.defaultValue) {
								tmp = $.isFunction(opt.defaultValue) ? opt
							...
							elc = $.jgrid.createEl(this.edittype, opt, tmp,
									false, $.extend({}, $.jgrid.ajaxOptions,
							...
							if (frmopt.rowabove) {
								var newdata = $("<tr><td class='contentinfo' colspan='"
										+ (maxcols * 2)
										+ "'>"
										+ frmopt.rowcontent + "</td></tr>");
								$(tb).append(newdata);
								newdata[0].rp = rp;
							}
							if (trdata.length === 0) {
								trdata = $("<tr " + dc + " rowpos='" + rp
										+ "'></tr>").addClass("FormData").attr(
										"id", "tr_" + nm);
								$(trdata).append(tmpl);
								$(tb).append(trdata);
								trdata[0].rp = rp;
							...
							$("td:eq(" + (cp - 2) + ")", trdata[0])
									.html(typeof frmopt.label === 'undefined'
											? obj.p.colNames[i]
											: frmopt.label);
							$("td:eq(" + (cp - 1) + ")", trdata[0])
									.append(frmopt.elmprefix).append(elc)
					...
					if (rowid == '_empty') {
						$(cm).each(function(i) {
							...
							if (fld && fld.length && fld[0] !== null) {
								vl = "";
								if (opt.defaultValue) {
									vl = $.isFunction(opt.defaultValue) ? opt
											.defaultValue() : opt.defaultValue;
				...
				function postIt() {
					...
					if ($.isFunction(rp_ge[$t.p.id].beforeCheckValues)) {
						var retvals = rp_ge[$t.p.id].beforeCheckValues(
								postdata, $("#" + frmgr), postdata[$t.p.id
										+ "_id"] == "_empty"
										? opers.addoper
										: opers.editoper);
					...
					if (ret[0]) {
						if ($.isFunction(rp_ge[$t.p.id].onclickSubmit)) {
							onCS = rp_ge[$t.p.id].onclickSubmit(rp_ge[$t.p.id],
									postdata)
									|| {};
						}
					...
					if (ret[0] && !rp_ge[$t.p.id].processing) {
						...
						oper = opers.oper;
						idname = opers.id;
						// we add to pos data array the action - the name is
						// oper
						postdata[oper] = ($.trim(postdata[$t.p.id + "_id"]) == "_empty")
								? opers.addoper
								: opers.editoper;
						...
						var ajaxOptions = $.extend({
							...
							data : $
									.isFunction(rp_ge[$t.p.id].serializeEditData)
									? rp_ge[$t.p.id]
							...
							complete : function(data, Status) {
								if (Status != "success") {
									...
									if ($
										...
										ret = rp_ge[$t.p.id].afterSubmit(data,
												postdata);
									}
								}
								if (ret[0] === false) {
									...
									// remove some values if formattaer select
									// or checkbox
									$.each($t.p.colModel, function(i, n) {
						...
						if (ret[0]) {
							if (rp_ge[$t.p.id].useDataProxy) {
								$t.p.dataProxy.call($t, ajaxOptions, "set_"
												+ $t.p.id);
							} else {
								$.ajax(ajaxOptions);
							}
				...
				function setNulls() {
					$.each($t.p.colModel, function(i, n) {
						if (n.editoptions && n.editoptions.NullIfEmpty === true) {
							if (postdata.hasOwnProperty(n.name)
									&& postdata[n.name] == "") {
				...
				if ($("#" + IDs.themodal).html() !== null) {
					...
					restoreInline();
					$(".ui-jqdialog-title", "#" + IDs.modalhead)
							.html(p.caption);
					$("#FormError", "#" + frmtb).hide();
					if (rp_ge[$t.p.id].topinfo) {
						$(".topinfo", "#" + frmtb + "_2")
								.html(rp_ge[$t.p.id].topinfo);
						$(".tinfo", "#" + frmtb + "_2").show();
					} else {
						$(".tinfo", "#" + frmtb + "_2").hide();
					}
					...
					var dh = isNaN(p.dataheight) ? p.dataheight : p.dataheight
							...
							+ dh + ";'></form>").data("disabled", false), tbl = $("<table id='"
							+ frmtb
					...
					$($t.p.colModel).each(function(i) {
						...
						maxCols = Math
								.max(maxCols, fmto ? fmto.colpos || 0 : 0);
						maxRows = Math
								.max(maxRows, fmto ? fmto.rowpos || 0 : 0);
					});
					$(frm).append(tbl);
					...
					// create data
					var rtlb = $t.p.direction == "rtl" ? true : false, bp = rtlb
							? "nData"
					...
					$.jgrid.createModal(IDs, tms, p, "#gview_" + $t.p.id,
							$("#gbox_" + $t.p.id)[0]);
					if (rtlb) {
						$("#pData, #nData", "#" + frmtb + "_2").css("float",
								"right");
						$(".EditButton", "#" + frmtb + "_2").css("text-align",
								"left");
					}
					if (rp_ge[$t.p.id].topinfo) {
						$(".tinfo", "#" + frmtb + "_2").show();
					...
					if (p.checkOnUpdate) {
						$("a.ui-jqdialog-titlebar-close span",
								"#" + IDs.themodal).removeClass("jqmClose");
						$("a.ui-jqdialog-titlebar-close", "#" + IDs.themodal)
					...
					p.saveicon = $.extend([true, "left", "ui-icon-disk"],
							p.saveicon);
					p.closeicon = $.extend([true, "left", "ui-icon-close"],
							p.closeicon);
					// beforeinitdata after creation of the form
					if (p.saveicon[0] === true) {
						$("#sData", "#" + frmtb + "_2")
								.addClass(p.saveicon[1] == "right"
										? 'fm-button-icon-right'
										: 'fm-button-icon-left')
								.append("<span class='ui-icon " + p.saveicon[2]
										+ "'></span>");
					}
					if (p.closeicon[0] === true) {
						$("#cData", "#" + frmtb + "_2")
								.addClass(p.closeicon[1] == "right"
										? 'fm-button-icon-right'
										: 'fm-button-icon-left')
					...
					$(".fm-button", "#" + IDs.themodal).hover(function() {
								$(this).addClass('ui-state-hover');
							}, function() {
								$(this).removeClass('ui-state-hover');
							});
					$("#sData", "#" + frmtb + "_2").click(function(e) {
						...
						// ret[2] - the id that will be set if reload after
						// submit false
						getFormData();
					...
					$("#nData", "#" + frmtb + "_2").click(function(e) {
						...
						if (npos[0] != -1 && npos[1][npos[0] + 1]) {
							if ($.isFunction(p.onclickPgButtons)) {
								p.onclickPgButtons('next', $("#" + frmgr),
										npos[1][npos[0]]);
							}
							fillData(npos[1][npos[0] + 1], $t, frmgr);
							$($t).jqGrid("setSelection", npos[1][npos[0] + 1]);
							if ($.isFunction(p.afterclickPgButtons)) {
								p.afterclickPgButtons('next', $("#" + frmgr),
										npos[1][npos[0] + 1]);
							}
					...
					$("#pData", "#" + frmtb + "_2").click(function(e) {
						...
						if (ppos[0] != -1 && ppos[1][ppos[0] - 1]) {
							if ($.isFunction(p.onclickPgButtons)) {
								p.onclickPgButtons('prev', $("#" + frmgr),
										ppos[1][ppos[0]]);
							}
							fillData(ppos[1][ppos[0] - 1], $t, frmgr);
							$($t).jqGrid("setSelection", ppos[1][ppos[0] - 1]);
							if ($.isFunction(p.afterclickPgButtons)) {
								p.afterclickPgButtons('prev', $("#" + frmgr),
										ppos[1][ppos[0] - 1]);
							}
		...
		viewGridRow : function(rowid, p) {
			...
			return this.each(function() {
				...
				var gID = $t.p.id, frmgr = "ViewGrid_" + gID, frmtb = "ViewTbl_"
						+ gID, IDs = {
					themodal : 'viewmod' + gID,
					modalhead : 'viewhd' + gID,
					modalcontent : 'viewcnt' + gID,
					scrollelm : frmgr
				}, onBeforeInit = $.isFunction(p.beforeInitData)
						? p.beforeInitData
				...
				function createData(rowid, obj, tb, maxcols) {
					var nm, hc, trdata, cnt = 0, tmp, dc, retpos = [], ind = false, tdtmpl = "<td class='CaptionTD form-view-label ui-widget-content' width='"
							+ p.labelswidth
					...
					$(obj.p.colModel).each(function(i) {
						...
						if (!hc && this.align === 'right') {
							if (this.formatter
									&& $.inArray(this.formatter, fmtnum) !== -1) {
					...
					$(obj.p.colModel).each(function(i) {
						...
						dc = hc ? "style='display:none'" : "";
						viewfld = (typeof this.viewable != 'boolean')
								? true
								: this.viewable;
						if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn'
								&& viewfld) {
							...
							$("td:eq(" + (cp - 2) + ")", trdata[0]).html('<b>'
									+ (typeof frmopt.label === 'undefined'
											? obj.p.colNames[i]
											: frmopt.label) + '</b>');
							$("td:eq(" + (cp - 1) + ")", trdata[0])
									.append("<span>" + tmp + "</span>").attr(
											"id", "v_" + nm);
							if (setme) {
				...
				function fillData(rowid, obj) {
					...
					$('td', trv).each(function(i) {
						...
						if (obj.p.colModel[i].editrules
							...
							hc = obj.p.colModel[i].hidden === true
									? true
									: false;
						}
						if (nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
				...
				if ($("#" + IDs.themodal).html() !== null) {
					...
					var bt = $("<span></span>")
							...
							.append("<table border='0' class='EditTable' id='"
									...
									+ "</td></tr></tbody></table>");
					$.jgrid.createModal(IDs, bt, p, "#gview_" + $t.p.id,
							$("#gview_" + $t.p.id)[0]);
					if (rtlb) {
						$("#pData, #nData", "#" + frmtb + "_2").css("float",
								"right");
					...
					$(".fm-button:not(.ui-state-disabled)", "#" + frmtb + "_2")
							.hover(function() {
										$(this).addClass('ui-state-hover');
									}, function() {
										$(this).removeClass('ui-state-hover');
		...
		delGridRow : function(rowids, p) {
			...
			return this.each(function() {
				...
				var onBeforeShow = $.isFunction(rp_ge[$t.p.id].beforeShowForm), onAfterShow = $
						.isFunction(rp_ge[$t.p.id].afterShowForm), onBeforeInit = $
						.isFunction(rp_ge[$t.p.id].beforeInitData)
						? rp_ge[$t.p.id].beforeInitData
						: false, gID = $t.p.id, onCS = {}, showFrm = true, dtbl = "DelTbl_"
						+ gID, postd, idname, opers, oper, IDs = {
					themodal : 'delmod' + gID,
					modalhead : 'delhd' + gID,
				...
				if ($("#" + IDs.themodal).html() !== null) {
					...
					p.gbox = "#gbox_" + gID;
					$.jgrid.createModal(IDs, tbl, p, "#gview_" + $t.p.id,
							$("#gview_" + $t.p.id)[0]);
					...
					p.delicon = $.extend([true, "left", "ui-icon-scissors"],
							rp_ge[$t.p.id].delicon);
					p.cancelicon = $.extend([true, "left", "ui-icon-cancel"],
							rp_ge[$t.p.id].cancelicon);
					if (p.delicon[0] === true) {
						$("#dData", "#" + dtbl + "_2")
								.addClass(p.delicon[1] == "right"
										? 'fm-button-icon-right'
										: 'fm-button-icon-left')
								.append("<span class='ui-icon " + p.delicon[2]
										+ "'></span>");
					}
					if (p.cancelicon[0] === true) {
						$("#eData", "#" + dtbl + "_2")
								.addClass(p.cancelicon[1] == "right"
										? 'fm-button-icon-right'
										: 'fm-button-icon-left')
								.append("<span class='ui-icon "
										+ p.cancelicon[2] + "'></span>");
					...
					$("#dData", "#" + dtbl + "_2").click(function(e) {
						...
						if (ret[0] && !rp_ge[$t.p.id].processing) {
							...
							var ajaxOptions = $.extend(
									{
										...
										type : rp_ge[$t.p.id].mtype,
										data : $
												.isFunction(rp_ge[$t.p.id].serializeDelData)
												? rp_ge[$t.p.id]
														.serializeDelData(postd)
												: postd,
										complete : function(data, Status) {
											if (Status != "success") {
												ret[0] = false;
							...
							if (ret[0]) {
								if (rp_ge[$t.p.id].useDataProxy) {
									$t.p.dataProxy.call($t, ajaxOptions, "del_"
													+ $t.p.id);
								} else {
									$.ajax(ajaxOptions);
								}
		...
		navButtonAdd : function(elem, p) {
			...
			return this.each(function() {
				...
				// edit by fengmq 2011/12/31，将按钮加到grid的上面
				...
				if (findnav) {
					...
					if (p.position == 'first') {
						if (findnav.rows[0].cells.length === 0) {
							...
							$("tr td:eq(0)", findnav)
									.after("<td class='ui-pg-button ui-state-disabled' style='width:4px;'><span class='ui-separator'></span></td>");
						}
					} else {
						$("tr", findnav).append(tbd);
					}
			...
			return this.each(function() {
						...
						if (rowdata) {
							for (var i in rowdata) {
								if ($("[name=" + $.jgrid.jqID(i) + "]", formid)
										.is("input:radio")
										|| $("[name=" + $.jgrid.jqID(i) + "]",
												formid).is("input:checkbox")) {
...
(function($) {
	...
	$.jgrid.extend({
					...
					$('td', ind).each(function(i) {
						...
						if (nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
							...
							if (cm[i].editable === true) {
								...
								var elc = $.jgrid.createEl(cm[i].edittype, opt,
										tmp, true, $.extend({},
												$.jgrid.ajaxOptions,
												$t.p.ajaxSelectOptions || {}));
		...
		saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,
			...
			if (editable === "1") {
				...
				$("td", ind).each(function(i) {
					cm = $t.p.colModel[i];
					nm = cm.name;
					if (nm != 'cb' && nm != 'subgrid' && cm.editable === true
							&& nm != 'rn'
						...
						switch (cm.edittype) {
							case "checkbox" :
								...
								tmp[nm] = $("input", this).attr("checked")
										? cbv[0]
										: cbv[1];
								break;
							case 'text' :
							...
							case 'select' :
								if (!cm.editoptions.multiple) {
									tmp[nm] = $("select>option:selected", this)
											.val();
									tmp2[nm] = $("select>option:selected", this)
											.text();
								} else {
						...
						if (o.url !== 'clientArray' && cm.editoptions
					...
					try {
						var positions = $.jgrid
								.findPos($("#" + $.jgrid.jqID(rowid),
										$t.grid.bDiv)[0]);
						$.jgrid.info_dialog($.jgrid.errors.errcap, cv[1],
								$.jgrid.edit.bClose, {
									left : positions[0],
									top : positions[1]
								});
			...
			return this.each(function() {
				...
				if (fr >= 0) {
					if ($.isFunction($.fn.datepicker)) {
						try {
							$("input.hasDatepicker", "#" + $.jgrid.jqID(ind.id))
									.datepicker('hide');
						} catch (e) {
						}
					}
	...
	$.jgrid.extend({
			...
			return this.each(function() {
				...
				if (cm.editable === true && ed === true
					...
					if (parseInt($t.p.iCol, 10) >= 0
						...
						$("td:eq(" + $t.p.iCol + ")", $t.rows[$t.p.iRow])
					...
					if ($.isFunction($t.p.formatCell)) {
						var tmp2 = $t.p.formatCell.call($t, $t.rows[iRow].id,
								nm, tmp, iRow, iCol);
						if (tmp2 !== undefined) {
							tmp = tmp2;
						}
					...
					var elc = $.jgrid.createEl(cm.edittype, opt, tmp, true, $
									.extend({}, $.jgrid.ajaxOptions,
											$t.p.ajaxSelectOptions || {}));
					if ($.isFunction($t.p.beforeEditCell)) {
						$t.p.beforeEditCell.call($t, $t.rows[iRow].id, nm, tmp,
								iRow, iCol);
					}
							...
							function(e) {
								...
								if (e.keyCode == 9) {
									if (!$t.grid.hDiv.loading) {
										if (e.shiftKey) {
											$($t)
													.jqGrid("prevCell", iRow,
															iCol);
										} // Shift TAb
										else {
											$($t)
													.jqGrid("nextCell", iRow,
															iCol);
										} // Tab
									} else {
										return false;
					...
					if ($.isFunction($t.p.afterEditCell)) {
						$t.p.afterEditCell.call($t, $t.rows[iRow].id, nm, tmp,
								iRow, iCol);
					}
				} else {
					if (parseInt($t.p.iCol, 10) >= 0
					...
					if ($.isFunction($t.p.onSelectCell)) {
						...
						$t.p.onSelectCell.call($t, $t.rows[iRow].id, nm, tmp,
								iRow, iCol);
					}
				}
				$t.p.iCol = iCol;
		...
		saveCell : function(iRow, iCol) {
			return this.each(function() {
				...
				if (fr !== null) {
					var cc = $("td:eq(" + iCol + ")", $t.rows[iRow]), v, v2, cm = $t.p.colModel[iCol], nm = cm.name, nmjq = $.jgrid
							.jqID(nm);
					switch (cm.edittype) {
						case "select" :
							if (!cm.editoptions.multiple) {
						...
						case "checkbox" :
							...
							v = $("#" + iRow + "_" + nmjq, $t.rows[iRow])
									.attr("checked") ? cbv[0] : cbv[1];
							v2 = v;
							break;
						case "password" :
						case "text" :
						...
						case 'custom' :
							try {
								if (cm.editoptions
										&& $
												.isFunction(cm.editoptions.custom_value)) {
									v = cm.editoptions.custom_value.call($t, $(
													".customelement", cc),
											'get');
									if (v === undefined) {
										throw "e2";
					...
					// The common approach is if nothing changed do not do
					// anything
					if (v2 !== $t.p.savedRow[fr].v) {
						if ($.isFunction($t.p.beforeSaveCell)) {
							var vv = $t.p.beforeSaveCell.call($t,
						...
						if (cv[0] === true) {
							...
							if ($.isFunction($t.p.beforeSubmitCell)) {
								addpost = $t.p.beforeSubmitCell.call($t,
										$t.rows[iRow].id, nm, v, iRow, iCol);
								if (!addpost) {
									addpost = {};
								}
									...
									$.ajax($.extend(
											{
												url : $t.p.cellurl,
												data : $
														.isFunction($t.p.serializeCellData)
														? $t.p.serializeCellData
																.call($t,
																		postdata)
														: postdata,
												type : "POST",
		...
		GridNav : function() {
			return this.each(function() {
				...
				$t.p.knv = $t.p.id + "_kn";
				var selection = $("<span style='width:0px;height:0px;background-color:black;' tabindex='0'><span tabindex='-1' style='width:0px;height:0px;background-color:grey' id='"
						+ $t.p.knv + "'></span></span>"), i, kdir;
				...
				$("#" + $t.p.knv).focus().keydown(function(e) {
					...
					switch (kdir) {
						...
						case 13 :
							if (parseInt($t.p.iCol, 10) >= 0
									&& parseInt($t.p.iRow, 10) >= 0) {
								$($t).jqGrid("editCell", $t.p.iRow, $t.p.iCol,
										true);
							}
							break;
					}
...
(function($) {
	...
	$.jgrid.extend({
		...
		addSubGridCell : function(pos, iRow) {
			...
			return "<td role=\"grid\" aria-describedby=\"" + sid
		...
		addSubGrid : function(pos, sind) {
			return this.each(function() {
				...
				var subGridCell = function(trdiv, cell, pos) {
					var tddiv = $("<td align='"
							+ ts.p.subGridModel[0].align[pos] + "'></td>")
							.html(cell);
					$(trdiv).append(tddiv);
				};
					...
					if (sjxml) {
						...
						$(sgmap.root + " " + sgmap.row, sjxml).each(function() {
							...
							if (sgmap.repeatitems === true) {
								...
								if (f) {
									for (i = 0; i < f.length; i++) {
										subGridCell(trdiv, $(f[i], this).text()
														|| '&#160;', i);
									}
								}
							}
						...
						switch (ts.p.subgridtype) {
							...
							case "json" :
								$.ajax($.extend({
									...
									data : $
											.isFunction(ts.p.serializeSubGridData)
											? ts.p.serializeSubGridData.call(
													ts, dp)
											: dp,
						...
						$(ts.rows[i].cells[pos]).bind('click', function(e) {
							...
							if ($(this).hasClass("sgcollapsed")) {
								...
								if (ts.p.subGridOptions.reloadOnExpand === true
										|| (ts.p.subGridOptions.reloadOnExpand === false && !$(r)
												.hasClass('ui-subgrid'))) {
									atd = pos >= 1 ? "<td colspan='" + pos
											+ "'>&#160;</td>" : "";
									bfsc = true;
									...
									if ($.isFunction(ts.p.subGridRowExpanded)) {
										ts.p.subGridRowExpanded.call(ts, pID
														+ "_" + _id, _id);
									} else {
										populatesubgrid(tr);
									}
								...
								if ($.isFunction(ts.p.subGridRowColapsed)) {
									_id = tr.id;
									bfsc = ts.p.subGridRowColapsed.call(ts, pID
													+ "_" + _id, _id);
								}
								if (bfsc === false) {
									return false;
	...
	$.jgrid.extend({
				...
				while (i < len) {
					...
					if ($t.p.treeGridModel == 'nested') {
						if (!ldat[isLeaf]) {
							...
							rgt = parseInt(ldat[$t.p.treeReader.right_field],
									10);
							// NS Model
					...
					twrap = "<div class='tree-wrap tree-wrap-" + $t.p.direction
							+ "' style='width:" + (ident * 18) + "px;'>";
					twrap += "<div style='"
							+ ($t.p.direction == "rtl" ? "right:" : "left:")
							+ (lftpos * 18) + "px;' class='ui-icon ";
					...
					if (ldat[isLeaf] == "true" || ldat[isLeaf] === true) {
						twrap += ((ldat[icon] !== undefined && ldat[icon] !== "")
								? ldat[icon]
								: $t.p.treeIcons.leaf)
								+ " tree-leaf treeclick'";
						ldat[isLeaf] = true;
					...
					ldat[expanded] = ((ldat[expanded] == "true" || ldat[expanded] === true)
							? true
							: false)
							&& ldat[loaded];
					...
					$($t.rows[i].cells[expCol])
							.wrapInner("<span class='cell-wrapper" + lf
									+ "'></span>").prepend(twrap);

					if (curLevel !== parseInt($t.p.tree_root_level, 10)) {
						var pn = $($t).jqGrid('getNodeParent', ldat);
						expan = pn && pn.hasOwnProperty(expanded)
								? pn[expanded]
								: true;
						if (!expan) {
					...
					$($t.rows[i].cells[expCol]).find("div.treeclick").bind(
							"click", function(e) {
								var target = e.target || e.srcElement, ind2 = $(
										target, $t.rows).closest("tr.jqgrow")[0].id, pos = $t.p._index[ind2];
								if (!$t.p.data[pos][isLeaf]) {
									if ($t.p.data[pos][expanded]) {
										$($t).jqGrid("collapseRow",
												$t.p.data[pos]);
										$($t).jqGrid("collapseNode",
												$t.p.data[pos]);
									} else {
										$($t).jqGrid("expandRow",
												$t.p.data[pos]);
					...
					if ($t.p.ExpandColClick === true) {
						$($t.rows[i].cells[expCol]).find("span.cell-wrapper")
								.css("cursor", "pointer").bind("click",
										function(e) {
											var target = e.target
													|| e.srcElement, ind2 = $(
													target, $t.rows)
													.closest("tr.jqgrow")[0].id, pos = $t.p._index[ind2];
											if (!$t.p.data[pos][isLeaf]) {
												if ($t.p.data[pos][expanded]) {
													$($t).jqGrid("collapseRow",
															$t.p.data[pos]);
				...
				switch ($t.p.treeGridModel) {
					case 'nested' :
						...
						$($t.p.data).each(function(i) {
							if (parseInt(this[level], 10) === parseInt(
				...
				switch ($t.p.treeGridModel) {
					case 'nested' :
						var lftc = $t.p.treeReader.left_field, rgtc = $t.p.treeReader.right_field, levelc = $t.p.treeReader.level_field, lft = parseInt(
								rc[lftc], 10), rgt = parseInt(rc[rgtc], 10), level = parseInt(
								rc[levelc], 10);
		...
		SortTree : function(sortname, newDir, st, datefmt) {
			return this.each(function() {
				...
				var i, len, rec, records = [], $t = this, query, roots, rt = $(this)
						.jqGrid("getRootNodes");
				// Sorting roots
				query = $.jgrid.from(rt);
				...
				for (i = 0, len = roots.length; i < len; i++) {
					rec = roots[i];
					records.push(rec);
					$(this).jqGrid("collectChildrenSortTree", records, rec,
							sortname, newDir, st, datefmt);
				}
				$.each(records, function(index, row) {
							var id = $.jgrid.getAccessor(this,
									$t.p.localReader.id);
							$('#' + $t.p.id + ' tbody tr:eq(' + index + ')')
									.after($('tr#' + id, $t.grid.bDiv));
						});
				query = null;
				roots = null;
				records = null;
		...
		collectChildrenSortTree : function(records, rec, sortname, newDir, st,
				datefmt) {
			return this.each(function() {
						if (!this.grid || !this.p.treeGrid) {
							return;
		...
		delTreeNode : function(rowid) {
			return this.each(function() {
				var $t = this, rid = $t.p.localReader.id, left = $t.p.treeReader.left_field, right = $t.p.treeReader.right_field, myright, width, res, key;
				if (!$t.grid || !$t.p.treeGrid) {
					return;
				}
				...
				if (rc !== undefined) {
					...
					if ($t.p.treeGridModel === "nested") {
						...
						if (res.length) {
							for (key in res) {
								res[key][left] = parseInt(res[key][left], 10)
										- width;
							}
						}
						...
						if (res.length) {
							for (key in res) {
								res[key][right] = parseInt(res[key][right], 10)
										- width;
							}
						}
					}
		...
		addChildNode : function(nodeid, parentid, data) {
			...
			if (data) {
				...
				var expanded = $t.p.treeReader.expanded_field, isLeaf = $t.p.treeReader.leaf_field, level = $t.p.treeReader.level_field, icon = $t.p.treeReader.icon_field, parent = $t.p.treeReader.parent_id_field, left = $t.p.treeReader.left_field, right = $t.p.treeReader.right_field, loaded = $t.p.treeReader.loaded, method, parentindex, parentdata, parentlevel, i, len, max = 0, rowind = parentid, leaf, maxright;
				...
				if ($t.p.treeGridModel === "nested") {
					...
					if (parentid !== null) {
						...
						if (res.length) {
							for (key in res) {
								res[key][left] = res[key][left] > maxright
										? parseInt(res[key][left], 10) + 2
										: res[key][left];
								res[key][right] = res[key][right] >= maxright
						...
						maxright = parseInt($($t).jqGrid('getCol', right,
										false, 'max'), 10);
				...
				if (parentid === null
						|| $($t).jqGrid("isNodeLoaded", parentdata) || leaf) {
...
(function($) {
	$.jgrid.extend({
		groupingSetup : function() {
			return this.each(function() {
				var $t = this, grp = $t.p.groupingView;
				if (grp !== null
						&& ((typeof grp === 'object') || $.isFunction(grp))) {
				...
				if (grp.groupSummary[0]) {
					$.each(grp.summaryval[0][itm], function(i, n) {
								if ($.isFunction(this.st)) {
									this.v = this.st.call($t, this.v, this.nm,
											record);
								} else {
									this.v = $($t).jqGrid(
		...
		groupingToggle : function(hid) {
			this.each(function() {
				var $t = this, grp = $t.p.groupingView, strpos = hid
						.lastIndexOf('_'), uid = hid.substring(0, strpos + 1), num = parseInt(
						hid.substring(strpos + 1), 10)
						+ 1, minus = grp.minusicon, plus = grp.plusicon, tar = $("#"
						+ hid), r = tar.length ? tar[0].nextSibling : null, tarspan = $("#"
						+ hid + " span." + "tree-wrap-" + $t.p.direction), collapsed = false;
		...
		groupingRender : function(grdata, colspans) {
			return this.each(function() {
				var $t = this, grp = $t.p.groupingView, str = "", icon = "", hid, pmrtl = "", gv, cp, ii;
				...
				$.each(grp.sortitems[0], function(i, n) {
					...
					icon = "<span style='cursor:pointer;' class='ui-icon "
							+ pmrtl + "' onclick=\"jQuery('#" + $t.p.id
							+ "').jqGrid('groupingToggle','" + hid
							+ "');return false;\"></span>";
					try {
						gv = $t.formatter(hid, grp.sortnames[0][i], cp,
								grp.sortitems[0]);
					} catch (egv) {
					...
					str += "<tr id=\""
							...
							+ "\">"
							+ icon
							+ $.jgrid.format(grp.groupText[0], gv,
									grdata[n].length) + "</td></tr>";
					...
					if (grp.groupSummary[0]) {
						...
						str += "<tr"
								...
								+ $t.p.direction + "\">";
						var fdata = grp.summaryval[0][n], cm = $t.p.colModel, vv, grlen = grdata[n].length;
						for (var k = 0; k < colspans; k++) {
							var tmpdata = "<td " + $t.formatCol(k, 1, '')
									+ ">&#160;</td>", tplfld = "{0}";
							$.each(fdata, function() {
		...
		groupingGroupBy : function(name, options, current) {
			return this.each(function() {
						...
						for (var i = 0; i < name.length; i++) {
							grp.visibiltyOnNextGrouping[i] = $("#" + $t.p.id
									+ "_" + name[i]).is(":visible");
						}
			...
			return this.each(function() {
						...
						if (current === true) {
							...
							$("tr.jqgroup, tr.jqfoot",
									"#" + $t.p.id + " tbody:first").remove();
							$("tr.jqgrow:hidden",
									"#" + $t.p.id + " tbody:first").show();
						} else {
							$($t).trigger("reloadGrid");
						}
					});
		...
		jqGridExport : function(o) {
			...
			this.each(function() {
						...
						if (gprm.treeGrid) {
							for (var key in gprm.treeReader) {
								if (gprm.treeReader.hasOwnProperty(key)) {
									gprm.colNames.splice(gprm.colNames.length
											- 1);
									gprm.colModel.splice(gprm.colModel.length
											- 1);
								}
							}
						}
						switch (o.exptype) {
							...
							case 'jsonstring' :
								ret = "{"
										+ xmlJsonClass.toJson(gprm, o.root,
												o.ident, false) + "}";
	...
	$.jgrid.extend({
		sortableColumns : function(tblrow) {
			return this.each(function() {
				...
				var sortable_opts = {
					...
					"axis" : "x",
					"scrollSensitivity" : "1",
					"items" : '>th:not(:has(#jqgh_' + tid + '_cb' + ',#jqgh_'
							+ tid + '_rn' + ',#jqgh_' + tid
							+ '_subgrid),:hidden)',
					"placeholder" : {
						element : function(item) {
							var el = $(document.createElement(item[0].nodeName))
									.addClass(item[0].className
					...
					"update" : function(event, ui) {
						var p = $(ui.item).parent(), th = $(">th", p), colModel = ts.p.colModel, cmMap = {}, tid = ts.p.id
								+ "_";
						...
						var permutation = [];
						th.each(function(i) {
									var id = $(">div", this).get(0).id.replace(
											/^jqgh_/, "").replace(tid, "");
									if (id in cmMap) {
		...
		columnChooser : function(opts) {
			...
			opts = $.extend({
				 ...
				 * multiselect, or a function that supports creating a
				 * multiselect object (with no argument, or when passed an
				 * object), and destroying it (when passed the string
				 * "destroy").
				 */
				"msel" : "multiselect",
				/* "msel_opts" : {}, */
				...
				/*
				 * dlog is either the name of a ui widget class that behaves in
				 * a dialog-like way, or a function, that supports creating a
				 * dialog (when passed dlog_opts) or destroying a dialog (when
				 * passed the string "destroy")
				 */
				"dlog" : "dialog",

				/*
				 * dlog_opts is either an option object to be passed to "dlog",
				 * or (more likely) a function that creates the options object.
			...
			if ($.ui) {
				if ($.ui.multiselect) {
					if (opts.msel == "multiselect") {
						...
						opts.msel_opts = $.extend($.ui.multiselect.defaults,
								opts.msel_opts);
					}
				}
			}
			...
			var dopts = $.isFunction(opts.dlog_opts) ? opts.dlog_opts.call(
					self, opts) : opts.dlog_opts;
			call(opts.dlog, selector, dopts);
			var mopts = $.isFunction(opts.msel_opts) ? opts.msel_opts.call(
			...
			return this.each(function() {
				...
				function updateDnD() {
					...
					$("tr.jqgrow:not(.ui-draggable)", $t).draggable($
							.isFunction(datadnd.drag) ? datadnd.drag.call(
				...
				if (typeof opts == 'string' && opts == 'updateDnD'
						&& $t.p.jqgdnd === true) {
					updateDnD();
					return;
				}
					...
					"drag" : function(opts) {
						return $.extend({
							start : function(ev, ui) {
								// if we are in subgrid mode try to collapse the
								// node
								if ($t.p.subGrid) {
									var subgid = $(ui.helper).attr("id");
									try {
										$($t).jqGrid('collapseSubGridRow',
												subgid);
									} catch (e) {
									}
								}
								// hack
								// drag and drop does not insert tr in table,
								// when the table has no rows
								// we try to insert new empty row on the
								// target(s)
								for (var i = 0; i < $.data($t, "dnd").connectWith.length; i++) {
									if ($($.data($t, "dnd").connectWith[i])
											.jqGrid('getGridParam', 'reccount') == "0") {
							...
							stop : function(ev, ui) {
								...
								// if we have a empty row inserted from start
								// event try to delete it
								for (var i = 0; i < $.data($t, "dnd").connectWith.length; i++) {
									$($.data($t, "dnd").connectWith[i]).jqGrid(
											'delRowData', 'jqg_empty_row');
								}
					...
					"drop" : function(opts) {
						return $.extend({
							accept : function(d) {
								...
								if (tid.length > 0
										&& $.data(tid[0], "dnd") !== undefined) {
									var cn = $.data(tid[0], "dnd").connectWith;
									return $.inArray('#' + this.id, cn) != -1
											? true
											: false;
								}
								return d;
							...
							drop : function(ev, ui) {
								...
								var getdata = ui.draggable.parent().parent()
										.jqGrid('getRowData', accept);
								if (!opts.dropbyname) {
									var j = 0, tmpdata = {}, dropname;
									var dropmodel = $("#" + this.id).jqGrid(
								...
								if (opts.beforedrop
									...
									// parameters to this callback - event,
									// element, data to be inserted, sender,
									// reciever
									// should return object which will be
									// inserted into the reciever
									var datatoinsert = opts.beforedrop.call(
											this, ev, ui, getdata, $('#'
													+ $t.id), $(this));
								...
								if (ui.helper.dropped) {
									...
									if (opts.autoid) {
										if ($.isFunction(opts.autoid)) {
											grid = opts.autoid.call(this,
													getdata);
										} else {
											grid = Math.ceil(Math.random()
													* 1000);
											grid = opts.autoidprefix + grid;
										}
									}
									// NULL is interpreted as undefined while
									// null as object
									$("#" + this.id).jqGrid('addRowData', grid,
											getdata, opts.droppos);
								}
				...
				for (var i = 0; i < opts.connectWith.length; i++) {
					...
					$(cn).droppable($.isFunction(opts.drop) ? opts.drop.call(
							$($t), opts) : opts.drop);
				}
			});
		},
		gridResize : function(opts) {
			return this.each(function() {
				...
				opts.stop = function(ev, ui) {
					$($t).jqGrid('setGridParam', {
						height : $("#gview_" + $t.p.id + " .ui-jqgrid-bdiv")
								.height()
					});
				...
				if (opts._alsoResize_) {
					var optstest = "{\'#gview_" + $t.p.id
							+ " .ui-jqgrid-bdiv\':true,'" + opts._alsoResize_
							+ "':true}";
					opts.alsoResize = eval('(' + optstest + ')'); // the only
																	// way that
																	// I found
																	// to do
																	// this
...
function tableToGrid(selector, options) {
	jQuery(selector).each(function() {
		...
		var inputCheckbox = jQuery(
				'tr td:first-child input[type=checkbox]:first', jQuery(this));
		var inputRadio = jQuery('tr td:first-child input[type=radio]:first',
				jQuery(this));
		...
		jQuery('tbody > tr', jQuery(this)).each(function() {
					...
					jQuery('td', jQuery(this)).each(function() {
								if (rowPos === 0 && selectable) {
									...
									row[colModel[rowPos].name] = jQuery(this)
											.html();
								}
								rowPos++;
							});
