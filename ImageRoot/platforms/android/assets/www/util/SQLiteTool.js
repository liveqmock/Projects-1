var SQLiteTool = {
	init : function(tx) {
		...
		tx.executeSql("CREATE TABLE IF NOT EXISTS imagestruc(_ID INTEGER PRIMARY KEY autoincrement, LotImgStruc_ID VARCHAR(64), LotImgStruc_ID_Parent VARCHAR(64), BD_ID VARCHAR(64), LotImgStrucCode VARCHAR(32), LotImgStrucName VARCHAR(64), SortCode VARCHAR(64), FullPath VARCHAR(512), NodeType VARCHAR(16), coordination VARCHAR(32), FOREIGN KEY(BD_ID) REFERENCES biaoduan(BD_ID))");
...
//					});
	...
	queryStrucByBDAndParent : function(db, bdid, parent, callback) {
		...
		db.transaction(function(tx) {
			console.log("queryStrucByBDAndParent bdid: " + bdid);
			var sql1;
			var sql11 = new Array();
			...
			if (bdid) {
				if((parent == "") || (parent == null)){
				   sql1 = "SELECT * FROM imagestruc where BD_ID = ? and LotImgStruc_ID_Parent IS NULL and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";
				   //sql1 = "select * from imagestruc where BD_ID = ? and  (_ID in(Select min(_ID) FROM imagestruc group by LotImgStrucName))";
				   sql11[0] = bdid;
				   //sql11[1] = "null";
				}else{
				   sql1 = "SELECT * FROM imagestruc where BD_ID = ? and LotImgStruc_ID_Parent = ? and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID)  order by SortCode";
				   sql11[0] = bdid;
				   sql11[1] = parent;
				}
				...
				sql1 = "SELECT * FROM imagestruc where _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";
			}
			
			/*tx.executeSql("delete FROM imagestruc",[],function(tx,r){
                                 console.log("delete imagestruc success : ");
                                 },function(e){
			...
			/*var sql12 = "SELECT * FROM imagestruc where BD_ID = ? and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode"; 
			var sql13 = new Array();
			sql13[0] = bdid;
			//sql13[1] = "4涵洞工程";
			tx.executeSql(sql12, sql13, function(tx, res) {
				console.log("sql12: " + sql12);
				console.log("获得【imagestruc】记录: " + res.rows.length + "条记录。");
				var result = new Array();
				for (var i = 0; i < res.rows.length; i++) {
					result[i] = res.rows.item(i);
					console.log('result[i].LotImgStrucName:'+result[i].LotImgStrucName);
					if(result[i].LotImgStrucName == '4涵洞工程'){
						console.log("涵洞工程 LotImgStrucName:" + result[i].LotImgStrucName + " LotImgStruc_ID_Parent:" + result[i].LotImgStruc_ID_Parent 
						+ " UpdateFlag:" + result[i].UpdateFlag);
					}
					//console.log("涵洞工程 LotImgStrucName:" + result[i].LotImgStrucName + " LotImgStruc_ID_Parent:" + result[i].LotImgStruc_ID_Parent);
				}
				//filterStructsInfo =  result;
				//callback(result);
			}, function(e) {
				console.log("ERROR: " + e.message);
			});*/
	...
	queryStrucBySortCode : function(db, bdid, sortCode, callback) {
		...
		db.transaction(function(tx) {
			console.log("queryStrucBySortCode bdid: " + bdid);
			var sql1;
			var sql11 = new Array();
			
			if (bdid && sortCode) {
				sql1 = "SELECT * FROM imagestruc where BD_ID = ? and SortCode like ? and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";
				sql11[0] = bdid;
				sql11[1] = sortCode + "%";
			} else if (bdid) {
				sql1 = "SELECT * FROM imagestruc where BD_ID = ? and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";
				sql11[0] = bdid;
			} else if (sortCode) {
				sql1 = "SELECT * FROM imagestruc where SortCode like ? and _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";
				sql11[0] = sortCode + "%";
			} else {
				sql1 = "SELECT * FROM imagestruc where _ID in(Select min(_ID) FROM imagestruc group by LotImgStruc_ID) order by SortCode";        
			}
			...
			tx.executeSql(sql1, sql11, function(tx, res) {
				...
				for (var i = 0; i < res.rows.length; i++) {
					...
					//console.log("queryStrucBySortCode result[i].LotImgStrucName:"+result[i].LotImgStrucName+" result[i].LotImgStruc_ID_Parent:"+result[i].LotImgStruc_ID_Parent);
					
				}
				//console.log("SQLiteTool queryStrucBySortCode : " + JSON.stringify(result));
				structsInfo = result;
				callback(result);
			}, function(e) {
				console.log("ERROR: " + e.message);
	...
	insertStruc : function(db, jsonArr) {
		db.transaction(function(tx) {
			var jsonInsert = new Array();
			if(jsonArr == 'yes'){
				//jsonInsert = imagestrucInfo;
				console.log("imagestrucInfo.length："+imagestrucInfo.length);
				for (var i = 0; i < imagestrucInfo.length; i++) {
						var json = imagestrucInfo[i];
						console.log("FullPath为：：" + json.FullPath);
						/*if(json.LotImgStruc_ID_Parent == null){
								console.log("json.LotImgStruc_ID_Paren为空：");
								json.LotImgStruc_ID_Parent = "";
						}*/
						if (0 == json.UpdateFlag) {
							// 插入
					...
					var bdidnow = TempCache.getItem("BdId");
				  	console.log("bdidnow:"+bdidnow + "-----------------");								  
				  	var d = new Date();
				 	var YMDHMS = d.getFullYear() + "-" +(d.getMonth()+1) + "-" + d.getDate() + " " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
				    
				 	SQLiteTool.upBDTable(db,bdidnow,YMDHMS,function()
				  	{
...
//                                     });
	...
	getCIM_LotImgStrucWithBdId : function(db, bdid, callback) {
		...
		db.transaction(function(tx) {
			//SQLiteTool.sleep(1000);
		  
			var sql1;
			var sql11 = new Array();
			console.log("getCIM_LotImgStrucWithBdId bdid: " + bdid);
			if (bdid) {
			...
			tx.executeSql(sql1, sql11, function(tx, res) {
				...
				/*for (var i = 0; i < resLength; i++) {
					imagestrucInfo[i] = res.rows.item(i);
				}*/
				doInsertStruc = 'yes';
				//SQLiteTool.insertStruc(db, doInsertStruc);
				
				var result;
				var resLength = res.rows.length;
				var intA = parseInt(resLength/1000);
				var resultIndex = 0;
				for (var i = 0; i < resLength; i++) {
					//result[resultIndex] = res.rows.item(i);
					resultIndex = 0;
					
					if((parseInt(i/1000) == (i/1000)) && ((i/1000)!=0)){
						result = new Array();
						for(h = (i-1000); h<i; h++){
							console.log("h: "+h);
							result[resultIndex] = res.rows.item(h);
							if(result[resultIndex].LotImgStrucName == '4涵洞工程'){
								nameInfo = result[resultIndex];
							}
							resultIndex++;	
						}
						if(res.rows.length>0){
							//console.log("res.rows.item(resultIndex-1).LotImgStruc_ID_Parent" + res.rows.item(resultIndex-1).LotImgStruc_ID_Parent);
						}
					...
					if(parseInt(i/1000)==intA){
						result = new Array();
						for(j = i; j<resLength; j++){
							console.log("j: "+j);
							result[resultIndex] = res.rows.item(j);
							if(result[resultIndex].LotImgStrucName == '4涵洞工程'){
								nameInfo = result[resultIndex];
							}
							resultIndex++;
						...
						//console.log("result[resultIndex].LotImgStruc_ID_Parent" + result[resultIndex-1].LotImgStruc_ID_Parent + " " +result[resultIndex-1].BD_ID);
						//SQLiteTool.insertStruc(db, result);
						i = resLength;
	...
	operateCIM_LotImgStruc : function(db, jsonArr, changeflag, callback) {
		db.transaction(function(tx) {
			...
			if(changeflag == 0){
				...
				var bdidnow = TempCache.getItem("BdId");
				console.log("bdidnow:"+bdidnow + "-----------------");
				SQLiteTool.getCIM_LotImgStrucWithBdId(db, bdidnow, function(backJson) {
