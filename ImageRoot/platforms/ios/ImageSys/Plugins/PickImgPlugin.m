...
- (void)imagePickerController:(QBImagePickerController *)imagePickerController didFinishPickingMediaWithInfo:(id)info
{
    if(imagePickerController.allowsMultipleSelection) {
        ...
        for (int i=0; i<[mediaInfoArray count]; i++) {
            ...
            UIImage *Img = [self scaleFromImage:tempImg toSize:CGSizeMake(tempImg.size.width/2, tempImg.size.height/2)];
            NSLog(@"pickImg.height:%f",Img.size.height);
            NSLog(@"pickImg.width:%f",Img.size.width);
            
            NSData * saveImgData = UIImageJPEGRepresentation(Img,0.1);
