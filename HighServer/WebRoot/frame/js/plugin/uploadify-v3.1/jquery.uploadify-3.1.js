...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg = '附件添加到上传队列失败:';
		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				/*if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('您选择的文件 "'
							+ file.name
							+ '" 已经上传到服务器.\n是否替换已存在文件?');
					if (!replaceQueueItem) {
			...
			if ($.inArray('onSelect', settings.overrideEvents) < 0) {
				...
				$('#' + settings.queueID)
					...
					<div class="cancel">\
						<a href="javascript:handleCancelId' + settings.id + '(\''+file.id+ '\',\'' + file.size + '\');$(\'#'
										+ settings.id
										+ '\').uploadify(\'cancel\', \''
										+ file.id
										+ '\')">X</a>\
